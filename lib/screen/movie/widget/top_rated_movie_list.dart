...
class TopRatedMovieList extends StatelessWidget {
  ...
  Widget build(BuildContext context) => BlocBuilder<MovieBloc, MovieState>(
        buildWhen: (previous, current) =>
            previous.topRatedCategoryMovies.results !=
            current.topRatedCategoryMovies.results,
        builder: (context, state) {
          final movies = state.topRatedCategoryMovies.results;
          return SizedBox(
            height: 300,
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                return _Item(item: movies[index]);
              },
